...
export async function fetchKarbonTasks() {
  ...
  // Active work only, 100 per page (Karbon's max); follow @odata.nextLink.
  ...
  for (let page = 0; page < 25 && url; page++) {
    // Out of time budget — return what we have rather than risk a kill.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error.code === 'KARBON_ERROR' || error.code === 'KARBON_NON_JSON') {
